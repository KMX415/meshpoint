...
class WismeshRadioCard {
    ...
    mount(root) {
        ...
        const broadcasts = document.createElement('fieldset');
        broadcasts.className = 'cfg-fieldset';
        broadcasts.hidden = true;
        broadcasts.dataset.pimeshBroadcasts = '';
        broadcasts.innerHTML = `<legend class="cfg-fieldset__legend">Native broadcasts</legend>
            <p class="cfg-field__hint">Meshtastic manages identity and device telemetry broadcasts.
            Leave an interval blank to keep its current firmware setting. Position sharing is configured under GPS.</p>
            <label class="cfg-field"><span class="cfg-field__label">Identity interval (minutes)</span>
                <input class="cfg-field__input" data-pimesh-nodeinfo type="number" min="5" max="1440"></label>
            <label class="cfg-field cfg-field--toggle"><input data-pimesh-telemetry-enabled type="checkbox">
                <span class="cfg-field__label">Device telemetry enabled</span></label>
            <label class="cfg-field"><span class="cfg-field__label">Telemetry interval (minutes)</span>
                <input class="cfg-field__input" data-pimesh-telemetry type="number" min="5" max="1440"></label>`;
        this._form.insertBefore(broadcasts, this._form.querySelector('.cfg-card__actions'));
    }

    render(config) {
        const pimesh = window.PlatformContext.isPimesh(config);
        this._moduleForm.closest('article').hidden = pimesh;
        if (pimesh) {
            this._root.querySelectorAll('.cfg-card__hint')[1].textContent = 'Configure the active Meshtastic radio. Settings are kept when you switch protocols.';
            this._txPower.max = '22';
        }
        ...
        this._pimesh = pimesh;
        this._root.querySelector('[data-pimesh-broadcasts]').hidden = !pimesh;
        this._root.querySelector('[data-pimesh-nodeinfo]').value = md.nodeinfo_interval_seconds ? md.nodeinfo_interval_seconds / 60 : '';
        this._root.querySelector('[data-pimesh-telemetry]').value = md.telemetry_interval_seconds > 0 && md.telemetry_interval_seconds <= 86400 ? md.telemetry_interval_seconds / 60 : '';
        this._root.querySelector('[data-pimesh-telemetry-enabled]').checked = Boolean(md.telemetry_enabled);
        this._root.querySelector('[data-pimesh-broadcasts]').disabled = !connected;
        this._broadcastInitial = md;
        ...
        this._fillRegions(pimesh
            ? [{ id: this._initial.region, name: this._initial.region }]
            : (config.regions || []));
        ...
        this._setStatus(
            this._statusEl,
            connected ? '' : (pimesh ? 'Radio disconnected. Check the PiMesh service status.'
                : 'Bridge disconnected. Apply module or restart meshtasticd, then Meshpoint.'),
            connected ? '' : 'error',
        );
    }
    ...
    async _onSubmit(event) {
        ...
        if (this._pimesh) {
            for (const [selector, key] of [['nodeinfo', 'nodeinfo_interval_seconds'], ['telemetry', 'telemetry_interval_seconds']]) {
                const value = this._root.querySelector(`[data-pimesh-${selector}]`).value;
                if (value && Number(value) * 60 !== this._broadcastInitial[key]) body[key] = Number(value) * 60;
            }
            const enabled = this._root.querySelector('[data-pimesh-telemetry-enabled]').checked;
            if (enabled !== this._broadcastInitial.telemetry_enabled) body.telemetry_enabled = enabled;
        }
