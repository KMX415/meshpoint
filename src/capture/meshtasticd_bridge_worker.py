...
class _BridgeWorker:
    ...
    def _on_receive(self, packet, interface) -> None:
        ...
        try:
            ...
            if raw_capture is not None:
                ...
                radio = read_radio_state_from_iface(interface)
                raw_capture.signal.frequency_mhz = radio.frequency_mhz or self._default_frequency_mhz
                raw_capture.signal.spreading_factor = radio.spreading_factor or 0
                raw_capture.signal.bandwidth_khz = radio.bandwidth_khz or 0
                try:
    ...
    def _handle_command(self, command: tuple[str, Any]) -> None:
        ...
        if op == BridgeCommand.SEND_TEXT:
            ...
            return
        if op == BridgeCommand.SEND_POSITION:
            try:
                if self._iface is None:
                    raise RuntimeError('Radio disconnected')
                values = {k: v for k, v in payload.items() if v is not None}
                sent = self._run_iface_op('sendPosition', lambda: self._iface.sendPosition(**values))
                self._resp_queue.put((BridgeResponse.OK, {'packet_id': f'{sent.id:08x}'}))
            except (OSError, RuntimeError, ValueError, AttributeError):
                self._resp_queue.put((BridgeResponse.ERROR, 'Position send failed'))
            return
    ...
    def _channels(self, op, payload):
        ...
        if node is None:
            ...
            return
        try:
            result = self._run_iface_op("channels", lambda: (
                read_channels(node) if op == BridgeCommand.READ_CHANNELS
                else write_channels(node, payload["channels"])))
            self._resp_queue.put((BridgeResponse.OK, result))
        except (ValueError, OSError, RuntimeError):
            self._resp_queue.put((BridgeResponse.ERROR, "Channel operation failed; reload to check device state"))
