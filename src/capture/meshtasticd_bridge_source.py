...
class MeshtasticdBridgeSource(CaptureSource):
    ...
    async def start(self) -> None:
        if self._connect_in_background:
            self._running = True
            return
    ...
    def request_send_position(self, latitude, longitude, altitude=None):
        return self._request(BridgeCommand.SEND_POSITION, {
            'latitude': latitude, 'longitude': longitude, 'altitude': altitude})

    def request_write_channels(self, channels):
        return self._request(BridgeCommand.WRITE_CHANNELS, {"channels": channels})
