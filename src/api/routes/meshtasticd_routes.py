...
def _require_node_platform() -> None:
    ...
    if _config.device.platform != "node" or (_config.device.radio_hat.startswith("pimesh-") and _config.device.radio_protocol != "meshtastic"):
        raise HTTPException(
            400,
...
class MeshtasticdRadioUpdate(BaseModel):
    ...
    nodeinfo_interval_seconds: int | None = Field(None, ge=300, le=86400)
    telemetry_interval_seconds: int | None = Field(None, ge=300, le=86400)
...
async def update_meshtasticd_radio(req: MeshtasticdRadioUpdate):
    ...
    if _config.device.radio_hat.startswith("pimesh-"):
        from src.radio.pimesh_supervisor import PROVISION, read_json

        if req.tx_power_dbm is not None and not 0 <= req.tx_power_dbm <= 22:
            raise HTTPException(400, "PiMesh chip power must be between 0 and 22 dBm")
        if req.region is not None and req.region != read_json(PROVISION).get("region"):
            raise HTTPException(400, "Region must match the provisioned PiMesh hardware")
    if not payload:
...
async def update_module_preset(req: ModulePresetUpdate):
    ...
    if _config.device.radio_hat.startswith("pimesh-"):
        raise HTTPException(409, "PiMesh hardware profile is managed by its installer")
