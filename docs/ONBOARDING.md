...
> **WisMesh Node (RAK6421 HAT, experimental):** On branch `feat/wismesh-hat`, use `sudo bash /opt/meshpoint/scripts/install.sh --platform node` instead of the gateway install above. The Node installer configures **meshtasticd** and installs the bundled **RAK13302 1W** LoRa preset by default (standard for WisBlock slot 1). See [`docs/plans/WISMESH-BRANCH.md`](plans/WISMESH-BRANCH.md).
